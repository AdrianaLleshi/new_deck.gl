define(["../../../lib/xeogl"], function () {
    ...
    xeogl.BIMCameraControl = xeogl.Component.extend({
        ...
        _init: function (cfg) {
            ...
            var rotationDeltas = this.rotationDeltas = math.vec2(); // Accumulated angle deltas while rotating with keyboard or mouse
            ...
            var resetRotate = this.resetRotate = function() {
            ...
            input.on("mousemove",
                function (canvasPos) {
                    ...
                    if (panning) {
                        ...
                        rotationDeltas[0] += (canvasPos[0] - lastCanvasPos[0]) / cw2 * 2 / math.DEGTORAD;
                        rotationDeltas[1] += (canvasPos[1] - lastCanvasPos[1]) / ch2 * 2;

                        math.rotationMat4v(rotationDeltas[1], orbitPitchAxis, pitchMat);

                        camera.view.eye = rotate(rotateStartEye);
                        camera.view.look = rotate(rotateStartLook);
                        camera.view.up = math.subVec3(rotate(rotateStartUp), camera.view.eye, math.vec3());
                    }
